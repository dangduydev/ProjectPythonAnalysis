...
class Window(cdd_tk.Tk):
    def __init__(self):
        super().__init__()
        self.CDD_FILE = os.path.abspath("") + "\CDD.mp3"
        ...
        self.btnExit.place(x=450, y=210, w=90, h=50)
        self.lb1 = cdd_tk.Label(
            self, text="Nhập yêu cầu cần thực hiện: ", font=("Arial Black", 10)
        )
        ...
        self.lbVoice = cdd_tk.Label(self, text="Bạn vừa nói", font=("Arial Black", 10))
    ...
    def game(self):
        subprocess.run(["python", "G3B14EX7_Game_08CaoDangDuy.py"])
    ...
    def Noi(self):
        ...
        with sr.Microphone() as Source:
            ...
            file_audio_data = rCDD.record(Source, duration=5)
            try:
                vCDDlenh = rCDD.recognize_google(file_audio_data, language=lang)
                if vCDDlenh == "hài" or vCDDlenh == "hi" or vCDDlenh == "ai":
                    self.on_closing
                if vCDDlenh == "một" or vCDDlenh == "mốt":
                    self.new_window_panel
                self.number = self.convert(vCDDlenh)
            ...
            self.CDD_FILE = "G337CDD.mp3"
            CDD_Text = gTTS(text=vCDDlenh, lang=lang)
            CDD_Text.save(self.CDD_FILE)
            self.Phat()
            self.ThucThi()

    def Phat(self):
        playsound.playsound(self.CDD_FILE)
...
class child_window(cdd_tk.Toplevel):
    def __init__(self, parent):
        ...
        self.forward_radio = cdd_tk.Radiobutton(
            self, text="Thuận", variable=self.direction_var, value="Thuận"
        )
        self.backward_radio = cdd_tk.Radiobutton(
            self, text="Nghịch", variable=self.direction_var, value="Nghịch"
        )
        ...
        self.btn_dropcolum = cdd_tk.Button(
            self, text="Xác nhận", command=lambda: self.valueWasChose(parent)
        )
    ...
    def on_closing(self):
        if cdd_tk.messagebox.askokcancel(
            "Đóng chương trình", "Bạn chắc chắn muốn thoát?"
        ):
            self.destroy()
    ...
    def valueWasChose(self, parent):
        ...
        parent.dropcolumn = all_items
        # print(parent.dropcolumn)
...
class new_panel(cdd_tk.Toplevel):
    def __init__(self, parent):
        ...
        self.btn = cdd_tk.Button(
            self, text="Drop Column", command=lambda: self.EAD_column(parent), width=10
        )
        ...
        self.btn_data = cdd_tk.Button(
            self, text="Thực hiện", command=lambda: self.EDA_data(parent), width=10
        )
        ...
        self.btn_row = cdd_tk.Button(
            self, text="Drop Row Null", command=lambda: self.EAD_row(parent), width=11
        )
        self.btn_row.place(x=1052, y=140)
        self.btn_dothi = cdd_tk.Button(
            self, text="Vẽ z-score", command=self.EDA_vedothi, width=10
        )
        self.btn_dothi.place(x=1052, y=180)
        self.btn_thaythe = cdd_tk.Button(
            self, text="Thay thế", command=lambda: self.EDA_thaythe(parent), width=10
        )
        ...
        self.btn_csv = cdd_tk.Button(self, text="Show", width=10, command=self.show_csv)
    ...
    def EDA_data(self, parent):
        ...
        if self.flag == 1:
            self.data = self.data.drop(columns=parent.dropcolumn, axis=1)
            self.text.insert(
                cdd_tk.END,
                "Kich thuoc cua DataFrame sau khi drop cot duoc chon: {}\n".format(
                    self.data.shape
                ),
            )
            self.text.insert(cdd_tk.END, str(self.data.count().sort_values()) + "\n")
        if self.flag == 2:
            ...
            self.data = self.data.dropna(subset=parent.dropcolumn)
            self.text.insert(
                cdd_tk.END,
                "Kich thuoc cua DataFrame sau khi drop cac dong Null theo cac cot da chon: {}\n".format(
                    self.data.shape
                ),
            )

            self.text.insert(cdd_tk.END, str(self.data.count().sort_values()) + "\n")
            self.text.insert(cdd_tk.END, "Tính toán Z core \n")
        ...
        if self.flag == 3:
            # Tính toán z-score cho các cột "Quantity" và "UnitPrice"
            z_scores = np.abs(stats.zscore(self.data[["Quantity", "UnitPrice"]]))

            # Vẽ biểu đồ z-score
            fig, ax = plt.subplots()
            ax.boxplot(z_scores, labels=["Quantity", "UnitPrice"])
            ax.set_ylabel("Z-Score")
            ax.set_title("Biểu đồ Z-Score trước khi xử lý ngoại lệ")
            plt.show()

            # Lọc dữ liệu theo điều kiện Z-Score
            ...
            ax.boxplot(z_scores, labels=["Quantity", "UnitPrice"])
            ax.set_ylabel("Z-Score")
            ax.set_title("Biểu đồ Z-Score sau khi xử lý ngoại lệ")
            plt.show()
        ...
        if self.flag == 4:
            mean_value = self.data[parent.dropcolumn].mean()
            self.data[parent.dropcolumn] = self.data[parent.dropcolumn].fillna(
                mean_value
            )
            self.text.insert(
                cdd_tk.END,
                "Kich thuoc cua DataFrame sau khi thay the cac dong null bang gia tri trung binh: {}\n".format(
                    self.data.shape
                ),
            )
            self.text.insert(cdd_tk.END, str(self.data.count().sort_values()) + "\n")
        ...
        if self.flag == 6:
            a = self.data.loc[:, self.data.columns != "Quantity"]
            b = self.data[["Quantity"]]
        ...
        if self.flag == 8:
            ...
            self.text.insert(cdd_tk.END, f"{str(parent.dropcolumn)} \n")
            for col in parent.dropcolumn:
...
class Do_Thi_CDD(cdd_tk.Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.title("Biểu đồ")
        ...
        self.lbl1 = cdd_tk.Label(
            self, text="Thống kê số lượng các sản phẩm bán chạy nhất"
        )
        self.lbl1.place(x=10, y=80)
        self.btn1 = cdd_tk.Button(self, text="Vẽ", command=self.veDoThi3, width=5)
        self.btn1.place(x=10, y=100)
        self.lbl2 = cdd_tk.Label(
            self, text="Thống kê số lượng các sản phẩm bán chạy nhất"
        )
        self.lbl2.place(x=10, y=140)
        self.btn2 = cdd_tk.Button(self, text="Vẽ", command=self.veDoThi4, width=5)
        self.btn2.place(x=10, y=160)

        self.data = pd.read_csv("G308CDD_DAHP_OnlineRetail.csv", encoding="ISO-8859-1")
    ...
    def veDoThi3(self):
        top_products = self.data["Description"].value_counts().head(10)
    ...
    def veDoThi4(self):
        revenue_by_country = self.data.groupby("Country")["Quantity", "UnitPrice"].sum()
        plt.figure(figsize=(10, 6))
        revenue_by_country["UnitPrice"].plot(kind="pie", autopct="%1.1f%%")
        plt.axis("equal")
        plt.title("Revenue Distribution by Country")
        plt.legend(revenue_by_country.index)
        plt.show()
